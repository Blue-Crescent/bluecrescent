...
public:
  BlueCrescent_head()
  {
    ...
    //signal(SIGINT, combined_robot_hw_tests::mySigIntHandler);
  ...
  //debug  fd_M1[1] = wiringPiI2CSetup(drv8830_addr_M1[1]);
    ...
    hardware_interface::JointStateHandle state_handle_head_roll("head_roll", &combined_robot_hw_tests::head_pos_[ROLL], &head_vel_[ROLL], &head_eff_[ROLL]);
    jnt_state_interface.registerHandle(state_handle_head_roll);
  
    hardware_interface::JointHandle pos_handle_head_roll(jnt_state_interface.getHandle("head_roll"), &combined_robot_hw_tests::head_cmd_[ROLL]);
    jnt_pos_interface.registerHandle(pos_handle_head_roll);
    
    hardware_interface::JointStateHandle state_handle_head_yaw("head_yaw", &combined_robot_hw_tests::head_pos_[YAW], &head_vel_[YAW], &head_eff_[YAW]);
    jnt_state_interface.registerHandle(state_handle_head_yaw);
  
    hardware_interface::JointHandle pos_handle_head_yaw(jnt_state_interface.getHandle("head_yaw"), &combined_robot_hw_tests::head_cmd_[YAW]);
